import { app } from './firebase.js?v=2.0.23';
import { state } from './state.js?v=2.0.23';
import { saveNewBottle } from './db.js?v=2.0.23';
...
export async function handleImageSelected(event) {
  ...
  try {
    ...
    try {
      ...
      geminiModel = getGenerativeModel(getVertexAI(app), { model: 'gemini-2.0-flash' });
    } catch (e1) {
      try {
        const { getAI, getGenerativeModel, GoogleAIBackend } =
          await import('https://www.gstatic.com/firebasejs/12.11.0/firebase-ai.js');
        geminiModel = getGenerativeModel(getAI(app, { backend: new GoogleAIBackend() }), { model: 'gemini-2.0-flash' });
      } catch (e2) {
        sdkLoadError = e2;
      }
    }
