import { db } from './firebase.js?v=2.0.57';
...
import { closeScanModal } from './ai.js?v=2.0.57';
  ...
  unsubscribeInventory = onSnapshot(
    ...
    async (snapshot) => {
      ...
      renderTimeout = setTimeout(async () => {
        const { renderInventory } = await import('./render.js?v=2.0.57');
        renderInventory();
      }, 50);
    },
    async (e) => {
      console.error('[Cellar] onSnapshot error:', e);
      const { showErrorToast, renderInventory } = await import('./render.js?v=2.0.57');
...
export async function deleteBottle(id) {
  if (!isOwner()) return;
  const { closeModalDirect, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.57');
  try {
...
export async function markConsumed(id) {
  if (!isOwner()) return;
  const { renderInventory, closeModalDirect, openModal, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.57');
...
export async function setRating(id, liked) {
  if (!isOwner()) return;
  const { renderInventory, openModal, showErrorToast } = await import('./render.js?v=2.0.57');
...
export async function toggleBuyAgain(id) {
  if (!isOwner()) return;
  const { openModal, showErrorToast } = await import('./render.js?v=2.0.57');
  try {
...
export async function saveNewBottle(data) {
  if (!isOwner()) return;
  const { openModal, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.57');
  try {
...
export async function updateConsumedCount(id, newCount) {
  if (!isOwner()) return;
  const { updateLastUpdatedUI, showErrorToast } = await import('./render.js?v=2.0.57');
  try {
...
export async function saveProScores(id, scoreData) {
  if (!isOwner()) return;
  const { openModal, updateLastUpdatedUI, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.57');
  try {
...
export async function bulkTagWineColor(onProgress) {
  ...
  const { guessWineColor } = await import('./ai.js?v=2.0.57');
