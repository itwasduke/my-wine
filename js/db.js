import { db } from './firebase.js?v=2.0.39';
...
import { closeScanModal } from './ai.js?v=2.0.39';
  ...
  unsubscribeInventory = onSnapshot(
    ...
    async (snapshot) => {
      ...
      renderTimeout = setTimeout(async () => {
        const { renderInventory } = await import('./ui.js?v=2.0.39');
        renderInventory();
      }, 50);
    },
    async (e) => {
      console.error('[Cellar] onSnapshot error:', e);
      const { showErrorToast, renderInventory } = await import('./ui.js?v=2.0.39');
...
export async function deleteBottle(id) {
  ...
  const { closeModalDirect, showErrorToast, showSuccessToast } = await import('./ui.js?v=2.0.39');
  try {
...
export async function markConsumed(id) {
  if (!state.currentUser) return;
  const { renderInventory, closeModalDirect, openModal, showErrorToast, showSuccessToast } = await import('./ui.js?v=2.0.39');
...
export async function setRating(id, liked) {
  if (!state.currentUser) return;
  const { renderInventory, openModal, showErrorToast } = await import('./ui.js?v=2.0.39');
...
export async function toggleBuyAgain(id) {
  if (!state.currentUser) return;
  const { openModal, showErrorToast } = await import('./ui.js?v=2.0.39');
  try {
...
export async function saveNewBottle(data) {
  if (!state.currentUser) return;
  const { openModal, showErrorToast, showSuccessToast } = await import('./ui.js?v=2.0.39');
  try {
...
export async function updateConsumedCount(id, newCount) {
  if (!state.currentUser) return;
  const { updateLastUpdatedUI, showErrorToast } = await import('./ui.js?v=2.0.39');
  try {
...
export async function saveProScores(id, scoreData) {
  const { openModal, updateLastUpdatedUI, showErrorToast, showSuccessToast } = await import('./ui.js?v=2.0.39');
  try {
...
export async function bulkUpdateScores(onProgress) {
  ...
  const { renderInventory: renderInv1 } = await import('./ui.js?v=2.0.39');
  renderInv1();
}
...
export async function bulkTagWineColor(onProgress) {
  ...
  const { guessWineColor } = await import('./ai.js?v=2.0.39');
  ...
  const { renderInventory: renderInv2 } = await import('./ui.js?v=2.0.39');
  renderInv2();
}
