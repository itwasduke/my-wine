import { db } from './firebase.js?v=2.0.61';
import { collection, getDocs, doc, updateDoc, addDoc, deleteDoc, deleteField, serverTimestamp, onSnapshot }
  from "https://www.gstatic.com/firebasejs/12.11.0/firebase-firestore.js";
import { state } from './state.js?v=2.0.61';
import { closeScanModal } from './ai.js?v=2.0.61';

const OWNER_UID = 'yV9R26u5U6YFqZ6p0uX0pY9R26u5';
  ...
  unsubscribeInventory = onSnapshot(
    ...
    async (snapshot) => {
      ...
      renderTimeout = setTimeout(async () => {
        const { renderInventory } = await import('./render.js?v=2.0.61');
        renderInventory();
      }, 50);
    },
    async (e) => {
      console.error('[Cellar] onSnapshot error:', e);
      const { showErrorToast, renderInventory } = await import('./render.js?v=2.0.61');
...
export async function deleteBottle(id) {
  if (!isOwner()) return;
  const { closeModalDirect, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.61');
  try {
...
export async function markConsumed(id) {
  ...
  if (!isOwner()) {
    ...
    const { showErrorToast } = await import('./render.js?v=2.0.61');
  ...
  console.log('[Cellar] Fetching dependencies from render.js...');
  const { renderInventory, closeModalDirect, openModal, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.61');
...
export async function setRating(id, liked) {
  if (!isOwner()) return;
  const { renderInventory, openModal, showErrorToast } = await import('./render.js?v=2.0.61');
...
export async function toggleBuyAgain(id) {
  if (!isOwner()) return;
  const { openModal, showErrorToast } = await import('./render.js?v=2.0.61');
  try {
...
export async function saveNewBottle(data) {
  if (!isOwner()) return;
  const { openModal, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.61');
  try {
...
export async function updateConsumedCount(id, newCount) {
  if (!isOwner()) return;
  const { updateLastUpdatedUI, showErrorToast } = await import('./render.js?v=2.0.61');
  try {
...
export async function saveProScores(id, scoreData) {
  if (!isOwner()) return;
  const { openModal, updateLastUpdatedUI, showErrorToast, showSuccessToast } = await import('./render.js?v=2.0.61');
  try {
...
export async function bulkUpdateScores(onProgress) {
  ...
  const { lookupProScores } = await import('./ai.js?v=2.0.61');
...
export async function bulkTagWineColor(onProgress) {
  ...
  const { guessWineColor } = await import('./ai.js?v=2.0.61');
