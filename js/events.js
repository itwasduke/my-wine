...
import { renderInventory, updateLastUpdatedUI } from './render.js?v=2.0.45';
import { openModal, closeModalDirect } from './modal.js?v=2.0.45';
...
export function initUIListeners() {
  ...
  navAnalytics.addEventListener('click', async () => {
    ...
    const { renderAnalytics } = await import('./analytics.js?v=2.0.45');
  ...
  navChangelog.addEventListener('click', async () => {
    ...
    const { renderChangelog } = await import('./renderChangelog.js?v=2.0.45');
  ...
  if (bulkUpdateBtn) {
    bulkUpdateBtn.addEventListener('click', async () => {
      ...
      const { bulkUpdateScores } = await import('./db.js?v=2.0.45');
  ...
  if (bulkColorBtn) {
    bulkColorBtn.addEventListener('click', async () => {
      ...
      if (bulkUpdateStatus) bulkUpdateStatus.style.display = 'block';
      const { bulkTagWineColor } = await import('./db.js?v=2.0.45');
  ...
  document.getElementById('modalContent').addEventListener('change', async e => {
    if (e.target.id === 'edit-consumed-count') {
      ...
      const { updateConsumedCount } = await import('./db.js?v=2.0.45');
  ...
  document.getElementById('modalContent').addEventListener('click', async e => {
    ...
    if (action === 'consume') {
      const { markConsumed } = await import('./db.js?v=2.0.45');
      markConsumed(id);
    } else if (action === 'qty-dec' || action === 'qty-inc') {
      const { updateQuantity } = await import('./db.js?v=2.0.45');
      ...
      const { saveProScores }   = await import('./db.js?v=2.0.45');
      try {
      ...
      const { setRating } = await import('./db.js?v=2.0.45');
      setRating(id, value === 'true');
    } else if (action === 'buy-again') {
      const { toggleBuyAgain } = await import('./db.js?v=2.0.45');
      toggleBuyAgain(id);
    } else if (action === 'delete') {
      const { confirmDeleteBottle } = await import('./db.js?v=2.0.45');
